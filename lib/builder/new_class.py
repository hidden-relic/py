...
class Framing:
	...
	def buildRoofTab(self):
		def buildRoofInput(self):
		  	...
		  	def buildOther(self):
		  		...
		  		self.roof_pitch_lbl = tk.Label(self.roof_other_frame, text = 'Pitch: ', font = self.myfont)
		  		self.roof_pitch_lbl.pack(side = 'left', padx=12, pady = 12)
		  		self.roof_pitch = tk.Entry(self.roof_other_frame, width=4, font=self.myfont, justify='center')
		  		self.roof_pitch.pack(side = 'left', padx=12, pady=12)
		  		self.roof_pitch.insert(0, 9)
		  		
		  		self.roof_tail = tk.Entry(self.roof_other_frame, width=4,font=self.myfont, justify='center')
		  		self.roof_tail.pack(side = 'right', padx=12, pady=12)
		  		self.roof_tail.insert(0, 12)
		  		self.roof_tail_lbl = tk.Label(self.roof_other_frame, text = 'Soffit: ', font = self.myfont)
	...
	def buildDeckTab(self):
		def buildDeckInput(self):
		  	...
		  	self.deck_depth_input_frame=tk.Frame(self.deck_input_tab)
		  	self.deck_depth_input_frame.pack()
		  	...
		  	self.deck_depth_lbl_frame = tk.Frame(self.deck_depth_input_frame)
		  	tk.Label(self.deck_depth_lbl_frame, text = "Building depth:", font = self.myfont).pack(side = 'top', padx=12, pady=1)
		  	self.deck_depth_lbl_frame.pack(side = 'top')
			  ...
			  def buildDepth(self):
			  	self.deck_depth_measurements_frame = tk.Frame(self.deck_depth_input_frame)
			  		
			  		def buildDFt(self):
			  		 	self.deck_depth_ft_frame = tk.Frame(self.deck_depth_measurements_frame)
			  		 	self.deck_depth_ft_frame.grid(column = 0, row = 0, sticky = 'w')
			  		 	tk.Label(self.deck_depth_ft_frame, text = "ft.", font = self.myfont).pack(padx = 6, ipadx = 12)
			  		 	self.deck_depth_ft = ttk.Combobox(self.deck_depth_ft_frame, height = 20, width = 6, values = tuple(self.vals['ft']), font = self.myfont, state = 'readonly')
			  		 	self.deck_depth_ft.pack(padx = 6, pady=12, ipadx = 12)
			  		 	self.deck_depth_ft.set(10)
			  		def buildDIn(self):
			  			self.deck_depth_in_frame = tk.Frame(self.deck_depth_measurements_frame)
			  			self.deck_depth_in_frame.grid(column = 1, row = 0)
			  			tk.Label(self.deck_depth_in_frame, text = "in.", font = self.myfont).pack(padx = 6, ipadx = 12)
			  			self.deck_depth_in = ttk.Combobox(self.deck_depth_in_frame, height = 20, width = 6, values = tuple(self.vals['in']), font = self.myfont, state = 'readonly')
			  			self.deck_depth_in.pack(padx = 6, pady=12, ipadx = 12)
			  			self.deck_depth_in.set(0)
			  		def buildDFr(self):
			  			self.deck_depth_fr_frame = tk.Frame(self.deck_depth_measurements_frame)
			  			self.deck_depth_fr_frame.grid(column = 2, row = 0, sticky = 'e')
			  			tk.Label(self.deck_depth_fr_frame, text = bytes.fromhex('BE'), font = self.myfont).pack(padx = 6, ipadx = 12)
			  			self.deck_depth_fr = ttk.Combobox(self.deck_depth_fr_frame, height = 20, width = 6, values = ("0", "1/16", "1/8", "3/16", "1/4", "5/16", "3/8", "7/16", "1/2", "9/16", "5/8", "11/16", "3/4", "13/16", "7/8", "15/16"), font = self.myfont, state = 'readonly')
			  			self.deck_depth_fr.pack(padx = 6, pady=12, ipadx = 12)
			  			self.deck_depth_fr.set(0)
			  		buildDFt(self)
		  			buildDIn(self)
		  			buildDFr(self)
	
		  		    self.deck_depth_measurements_frame.pack(side = 'top')
		  		    self.deck_depth_input_frame.pack(fill = 'x')
		  		    ttk.Separator(self.deck_depth_input_frame, orient='horizontal').pack(fill='both', pady=12)
