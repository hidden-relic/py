...
class Framing:
	...
	def buildMeasurementInput(self, parent, text, ft=True, inch=True, fr=True, defFt=0, defIn=0):
		...
		def buildMFt(self):
			...
			tk.Label(ft_frame, text="ft.", font=self.infofont, fg='#0088ff').pack(padx=6, ipadx=6)
		...
		def buildMIn(self):
			...
			tk.Label(in_frame, text="in.", font=self.infofont, fg='#0088ff').pack(padx=6, ipadx=6)
		...
		def buildMFr(self):
			...
			tk.Label(fr_frame, text=bytes.fromhex("BE"), font=self.infofont, fg='#0088ff').pack(padx=6, ipadx=6)
	...
	def x(self, t, n):
		self.g(t.xcor()+n, t.ycor())
	def y(self, t, n):
		self.g(t.xcor(), t.ycor()+n)
	...
	def drawRect(self, t, orient, width, length):
		self.east(t)
		_=self.f(t, width) if orient=='v' else self.f(length)
		self.south(t)
		_=self.f(t, length) if orient=='v' else self.f(width)
		self.west(t)
		_=self.f(t, width) if orient=='v' else self.f(length)
		self.north()
		_=self.f(t, length) if orient=='v' else self.f(width)
	...
	def buildNotebooks(self):
		def mainNotebook(self):
			...
			self.main_notebook.pack(side='top', fill='both')
...
#########################################
	...
	def roofDraw(self):
		self.x(self.roof_t, (self.root.winfo_screenwidth/2)-self.toScale(self.roof_ridge_inches))
		self.drawRect(self.roof_t, orient='v', width=self.toScale(self.roof_ridge_inches), length=self.toScale(12*12))
